...
def angleSet_Finite(p, quadrants=1):
    '''
    ...
    fareyVectors.generateFiniteWithCoverage(p)
    
    if quadrants > 1:
        quadrantVectors = []
        for vector in fareyVectors.vectors:
            p, q = farey.get_pq(vector)
            if q*p != 0:
                quadrantVectors.append(farey.farey(p, -q))
        fareyVectors.vectors = fareyVectors.vectors + quadrantVectors
    
    return fareyVectors.vectors
