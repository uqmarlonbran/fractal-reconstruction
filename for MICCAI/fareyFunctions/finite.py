...
#fractal creation
def finiteFractal(N, K, sortBy='Euclidean', twoQuads=True, centered=False, random=False):
    '''
    ...
    kSpace = np.zeros((N,N))
    if random:
        lines, angles, mValues = computeRandomLines(kSpace, anglesSorted, finiteAnglesSorted, K, centered, twoQuads)
    else:
...
#    print "ISUM:", Isum
    ...
    if mValues:
        ...
        mList = range(0, len(bins))
    #Set slices (0 <= m <= N)
    for k in mList: #iterate per row
        row = bins[k, :]
        slice = fftpack.fft(row)
        radon.setSlice(k,dftSpace,slice,p)
